...
from utils.ASG import *
from utils.train_util import *
from utils.contra_loss import *

from networks.vnet_AMC import VNet_AMC
from test import test_LA_Pancreas
...
def get_arguments():
    ...
    parser.add_argument('--mpmode', type=str, default='intensity', help='how to generate multiple prototypes')
    parser.add_argument('--pnum', type=int, default=3, help='the number of multiple prototypes')
...
def create_dataloader():
    if 'LA' in args.data_dir:
        ...
        train_labset = BraTS2019(args.data_dir, args.list_dir,split='lab', aug_times = args.aug_times)
        train_unlabset = BraTS2019(args.data_dir,args.list_dir,split='unlab', aug_times = args.aug_times)
        testset = BraTS2019(args.data_dir,args.list_dir,split='val', aug_times = args.aug_times) 
    else:
...
def main():
    ...
    contra_proto_loss = PrototypeContrastiveLoss(temperature=0.1)
    asg = ASGPrototype(min_pixels=100, max_prototypes=args.pnum, mode= args.mpmode)
    ...
    for epoch in tqdm(range(args.num_epochs), ncols=70):
        ...
        for step, (labeled_batch, unlabeled_batch) in enumerate(zip(trainlab_loader,trainunlab_loader)):
            ...
            lab_prototypes = asg.getAdaptivePrototypes(lab_img, lab_fts, lab_lab_onehot)
            ...
            with torch.no_grad():
                ...
                reliability_map = (1 - norm_uncertainty) 
                ...
                unlab_prototypes= asg.getAdaptivePrototypes(unlab_img, unlab_ema_fts, unlab_ema_mask_onehot, reliability_map)
                # print(unlab_prototypes.shape)
            ...
            lab_dist =  torch.stack([asg.calDist(lab_fts, prototype, scaler=args.scaler) for prototype in prototypes], dim=1)
            unlab_dist = torch.stack([asg.calDist(unlab_ema_fts, prototype, scaler=args.scaler) for prototype in prototypes], dim=1)
                
            '''Prototype consistency learning'''
            loss_pc_lab = ce_loss(lab_dist,lab_lab)
            loss_pc_unlab = torch.sum(pixel_level_ce_loss(unlab_dist, unlab_ema_mask) * reliability_map) / np.prod(np.array(norm_uncertainty.shape[-3:]))

            loss_contra_proto = contra_proto_loss(lab_prototypes, unlab_prototypes)

            '''Total'''
            loss = loss_supervised + loss_pc_lab + consistency_weight * loss_pc_unlab + consistency_weight * loss_contra_proto
            ...
            logging.info('epoch : %d, step : %d, loss_all: %.4f,'
                         ...
                         'loss_pc_unlab: %.4f, loss_contra_proto: %.4f, train_dice: %.4f' % (
                            epoch+1, step, loss.item(), 
                            loss_ce.item(), loss_dice.item(), loss_focal.item(), loss_iou.item(),
                            loss_supervised.item(), loss_pc_lab.item(), loss_pc_unlab.item(), loss_contra_proto.item(), train_dice))           
        ...
        # if True:
            ...
            if max_flag:
                ...
                np.save(os.path.join(save_path, 'best_epoch_prototypes.npy'), [p.detach().cpu().numpy() for p in prototypes])
        writer.flush()
