...
def test_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None,
    ...
    for image_path in image_list:
        ...
        if print_result:
            ...
            csv_writer.writerow([
                cnt + 1,  # Case_Number
                single_metric[0] * 100,  # Dice (percentage)
                single_metric[1] * 100,  # Jaccard (percentage)
                single_metric[2],  # HD95
                single_metric[3]   # ASD
            ])
        ...
        # save_result=True
        if save_result:
            nib.save(nib.Nifti1Image(prediction.astype(np.float32), np.eye(4)), os.path.join(test_save_path, str(cnt) + "_pred.nii.gz"))
            nib.save(nib.Nifti1Image(image[:].astype(np.float32), np.eye(4)),  os.path.join(test_save_path, str(cnt) + "_img.nii.gz"))
            nib.save(nib.Nifti1Image(label[:].astype(np.float32), np.eye(4)), os.path.join(test_save_path, str(cnt) + "_gt.nii.gz"))
        cnt += 1
